...
class Price_Predictor4:
    ...
    def input_params4(self, disp, mil, fuel, hei, leng, wid, powe, tor, seat_c, wheel):
        self.disp = disp
        self.mil = mil
        self.fuel = fuel
        self.hei = hei
    ...
    def xgb_fun4(self):
        ...
        df = df.loc[(df['price']>=5000000) & (df['price']<7500000)]
        X = df[['displacement', 'mileage', 'fuel_capacity', 'height', 'length', 'width', 
                'power', 'torque', 'seating_capacity', 'wheelbase']]
        y = df['price']
        ...
        xg_reg = xgb.XGBRegressor(objective ='reg:squarederror', colsample_bytree = 0.3,
                                  learning_rate = 0.1, max_depth = 1, alpha = 1, n_estimators = 600)
...
# pred = Price_Predictor4()


# In[4]:


# xg_reg, preds, y_test = pred.xgb_fun4()
